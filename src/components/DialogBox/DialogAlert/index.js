...
const App = ({ setOpen, open,input, pdfFile}) => {
  ...
  const submittedForm = async () => {
    ...
    formData.append("enrollment_form", blb);
    const signed1 = new Blob([], {
      type: "multipart/form-data",
    });

    formData.append("signed_form1", signed1);
   ...
   await fetch(
      `https://gs-stg.qa-enphaseenergy.com/enrollment-mgr/api/v1/application/add`,
      {
        method: "POST",
        headers: myHeaders,
        body:formData
